...
WEB_DISCLAIMER = (
    "A busca web compara fragmentos do texto com snippets fornecidos pelo mecanismo de busca. "
    "Os percentuais não representam comparação integral com as páginas e não constituem prova de plágio."
)
...
PROFILES = {
    "Padrão (equilibrado)": {"chunk_words": 60, "stride_words": 30, "threshold": 0.75, "top_k_per_chunk": 1},
    "Rigoroso (correspondência literal)": {"chunk_words": 80, "stride_words": 40, "threshold": 0.86, "top_k_per_chunk": 1},
    "Sensível (correspondência aproximada)": {"chunk_words": 40, "stride_words": 20, "threshold": 0.64, "top_k_per_chunk": 1},
}
...
def serpapi_search_chunk(chunk: str, key: str, num_results: int) -> List[Dict]:
    ...
    response = requests.get(
        ...
        params={"engine": "google", "q": query, "api_key": key, "num": num_results, "hl": "pt", "gl": "br"},
        timeout=25,
    )
...
def sequence_similarity(a: str, b: str) -> float:
    return difflib.SequenceMatcher(None, (a or "").lower(), (b or "").lower(), autojunk=False).ratio()
...
def web_similarity_scan(text: str, key: str, profile: Dict, num_chunks: int, num_results: int):
    ...
    for index, chunk in enumerate(chunks):
        for item in serpapi_search_chunk(chunk, key, num_results):
            ...
            if score >= 0.12:
                raw_hits.append(WebHit(item.get("title", "Resultado"), item.get("link", ""), snippet, score, chunk))
...
def analyze_linguistic_patterns(text: str) -> Dict:
    words = split_words(text)
    sentences = [s.strip() for s in re.split(r"[.!?]+", text or "") if len(split_words(s)) >= 3]
    paragraphs = [p.strip() for p in re.split(r"\n\s*\n", text or "") if p.strip()]
    if len(words) < 80 or len(sentences) < 4:
        return {
            "sufficient": False,
            "summary": "Texto insuficiente para uma análise linguística minimamente estável.",
            "display_metrics": {"Palavras": len(words), "Frases": len(sentences)},
            "observations": ["Recomenda-se utilizar pelo menos 80 palavras e quatro frases."],
        }
    ...
    connectors = ["além disso", "em suma", "portanto", "todavia", "nesse sentido", "por outro lado", "vale ressaltar", "conclui-se"]
...
def inject_css():
    st.markdown("""
    <style>
      .header {padding:1.25rem;background:#fff;border:1px solid #e5e7eb;border-radius:14px;text-align:center;margin-bottom:1rem}
      .card {background:#fff;padding:1.1rem;border:1px solid #e5e7eb;border-radius:12px;margin-bottom:.8rem}
      .note {background:#f8fafc;padding:.85rem;border-left:4px solid #64748b;border-radius:8px;margin:.6rem 0 1rem}
...
for key, default in {
    "library": {}, "local_result": None, "web_result": None, "ling_result": None,
...
selected = option_menu(
    None,
    ["Nova análise", "Biblioteca", "Busca web", "Padrões linguísticos", "Relatórios"],
    icons=["search", "folder", "globe", "body-text", "file-earmark-pdf"],
    orientation="horizontal",
)

with st.sidebar:
    st.header("Configurações")
    st.session_state.profile = st.selectbox("Perfil de comparação", list(PROFILES), index=list(PROFILES).index(st.session_state.profile))
...
def text_input_block(prefix: str):
    ...
    with tab2:
        uploaded = st.file_uploader("PDF, DOCX ou TXT", type=["pdf", "docx", "txt"], key=f"{prefix}_upload")
        if uploaded:
            try:
                ...
                with st.expander("Conferir texto extraído"):
                    st.text_area("Pré-visualização", uploaded_text[:12000], height=260, disabled=True, key=f"{prefix}_preview")
...
if selected == "Nova análise":
        ...
        if st.button("Comparar com a biblioteca", type="primary", disabled=not text):
            if not st.session_state.library:
                ...
                with st.spinner("Comparando os documentos..."):
                    similarity, matches = compute_matches(text, st.session_state.library, **profile)
                    total_chunks = count_possible_chunks(text, profile["chunk_words"], profile["stride_words"])
                    st.session_state.local_result = {
                        "name": name, "text": text, "similarity": similarity, "matches": matches,
                        "total_chunks": total_chunks, "documents": len(st.session_state.library),
                    }
    with right:
        ...
        if result:
            ...
            if score < .03:
                label, explanation = "Baixa similaridade", "Não foram encontradas correspondências relevantes na biblioteca selecionada."
            elif score < .25:
                label, explanation = "Similaridade limitada", "Alguns fragmentos apresentam correspondência e devem ser revisados no contexto."
            else:
                label, explanation = "Similaridade elevada", "Uma parcela relevante dos fragmentos analisados encontrou correspondência na biblioteca."
            st.markdown(f"<div class='card'><h2>{score*100:.1f}%</h2><b>{label}</b><p>{explanation}</p></div>", unsafe_allow_html=True)
            c1, c2, c3 = st.columns(3)
            c1.metric("Fragmentos avaliados", result["total_chunks"])
            c2.metric("Documentos", result["documents"])
            c3.metric("Correspondências", len(result["matches"]))
            st.caption("O percentual representa cobertura aproximada de fragmentos com correspondência, sem concluir plágio ou originalidade.")
    ...
    st.info("Os arquivos permanecem apenas na sessão atual. Não envie documentos sem autorização ou que contenham dados sensíveis desnecessários.")
    uploads = st.file_uploader("Adicionar documentos", type=["pdf", "docx", "txt"], accept_multiple_files=True, key="library_upload")
    ...
    for filename, content in list(st.session_state.library.items()):
        ...
        with c2:
            with st.popover("Visualizar"):
                st.text_area(filename, content[:15000], height=300, disabled=True, key=f"view_{filename}")
    ...
    profile = PROFILES[st.session_state.profile]
    max_possible = count_possible_chunks(text, profile["chunk_words"], profile["stride_words"]) if text else 0
    requested = st.slider("Quantidade máxima de fragmentos enviados", 1, 20, min(8, max(1, max_possible or 8)))
    preview = build_chunks(text, profile["chunk_words"], profile["stride_words"], requested) if text else []
    if preview:
        with st.expander("Visualizar exatamente os fragmentos que serão enviados"):
            for i, fragment in enumerate(preview, 1):
                st.write(f"**Fragmento {i}:** {fragment}")
    consent = st.checkbox("Revisei os fragmentos e confirmo que não contêm dados pessoais, clínicos, sigilosos ou conteúdo cuja transmissão não seja autorizada.")
    if st.button("Pesquisar na web", type="primary", disabled=not (text and consent)):
        ...
        if not key:
            ...
            try:
                ...
                st.session_state.web_result = {"name": name, "hits": hits, "searched": searched, "possible": max_possible}
            except Exception as exc:
                st.error(f"A busca não pôde ser concluída: {exc}")
    result = st.session_state.web_result
    if result:
        coverage = (result["searched"] / result["possible"] * 100) if result["possible"] else 0
        st.info(f"Foram pesquisados {result['searched']} de aproximadamente {result['possible']} fragmentos possíveis ({coverage:.1f}% de cobertura estimada).")
        if not result["hits"]:
            st.success("Nenhum resultado relevante foi encontrado nos snippets consultados.")
        for hit in result["hits"]:
            st.markdown(f"### [{hit.title}]({hit.link})")
            st.write(hit.snippet)
            st.caption(f"Proximidade com o snippet: {hit.score*100:.1f}%. Esse valor não representa comparação com a página completa.")
            st.divider()
    ...
    if st.button("Analisar padrões", type="primary", disabled=not text):
        st.session_state.ling_result = {"name": name, "result": analyze_linguistic_patterns(text)}
    data = st.session_state.ling_result
    if data:
        result = data["result"]
        st.markdown(f"<div class='card'><h3>Síntese</h3><p>{result['summary']}</p></div>", unsafe_allow_html=True)
    ...
    with c1:
        ...
        result = st.session_state.local_result
        if result:
            coverage = f"Foram avaliados aproximadamente {result['total_chunks']} fragmentos contra {result['documents']} documentos."
            pdf = generate_local_report(result["name"], result["similarity"], result["matches"], coverage, LOCAL_DISCLAIMER)
            st.download_button("Baixar PDF local", pdf, "veritas_comparacao_local.pdf", "application/pdf")
        else:
    ...
    with c2:
        ...
        result = st.session_state.web_result
        if result:
            coverage = f"Foram enviados {result['searched']} de aproximadamente {result['possible']} fragmentos possíveis."
            pdf = generate_web_report(result["name"], result["hits"], coverage, WEB_DISCLAIMER)
            st.download_button("Baixar PDF web", pdf, "veritas_busca_web.pdf", "application/pdf")
        else:
    ...
    with c3:
        ...
        data = st.session_state.ling_result
        if data:
            pdf = generate_linguistic_report(data["name"], data["result"], LINGUISTIC_DISCLAIMER)
            st.download_button("Baixar PDF linguístico", pdf, "veritas_padroes_linguisticos.pdf", "application/pdf")
        else:
            st.caption("Nenhuma análise disponível.")
